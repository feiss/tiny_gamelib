...
//  Call  `chasers_game(t, dt)` each frame.
...
function chasers_game(t, dt) {
    ...
    for (let x = 0; x < 30000; x++) {
        canvas.pset(floor(rnd() * W), floor(rnd() * H), floor(rnd() * 2));
    }
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        canvas.draw_line(enemy.x, enemy.y, enemy.x + arrow.x, enemy.y + arrow.y, 4)
        ...
        canvas.fill_circle(enemy.x, enemy.y, 5, 5);
    }

    for (const b of bullets) {
        canvas.fill_rect(b.x - 1, b.y - 1, 3, 3, 7);
        if (b.distance(player) < 5) {
            canvas.fill_rect(0, 0, W, H, 7);
        }
        b.add(b.dir);
    }

    canvas.fill_circle(player.x, player.y, 4, 6);
